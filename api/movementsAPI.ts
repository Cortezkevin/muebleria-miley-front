...
export const getAll = async (): Promise<SuccessResponseDTO<MovementsDTO[]> | ErrorResponseDTO> => {
  try{
    const { data } = await AxiosInstance.get<SuccessResponseDTO<MovementsDTO[]>>(PATH, headersWithToken);
    return data;
  }catch(e){
...
export const getById = async ( id: string ): Promise<SuccessResponseDTO<DetailedMovementsDTO> | ErrorResponseDTO> => {
  try{
    const { data } = await AxiosInstance.get<SuccessResponseDTO<DetailedMovementsDTO>>(PATH + "/" + id, headersWithToken);
    return data;
  }catch(e){
    return handleAPIError(e);
  } 
...
export const create = async ( movement: CreateInventoryMovementDTO ): Promise<SuccessResponseDTO<MovementsDTO[]> | ErrorResponseDTO> => {
  try{
    const { data } = await AxiosInstance.post<SuccessResponseDTO<MovementsDTO[]>>(PATH,movement, headersWithToken);
    return data;
  }catch(e){
    return handleAPIError(e);
  } 
...
export const update = async ( movement: UpdateInventoryMovementDTO ): Promise<SuccessResponseDTO<MovementsDTO> | ErrorResponseDTO> => {
  try{
    const { data } = await AxiosInstance.put<SuccessResponseDTO<MovementsDTO>>(PATH,movement, headersWithToken);
    return data;
  }catch(e){
    return handleAPIError(e);
  } 
